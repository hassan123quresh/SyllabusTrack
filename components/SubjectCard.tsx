...
export const SubjectCard = React.memo(({ subject, onToggleTopic, onAddTopic, onDeleteSubject, onDeleteTopic, onEditTopic }: SubjectCardProps) => {
  ...
  const donutStyle = { background: `conic-gradient(${subject.color} 0% ${progress}%, rgba(255,255,255,0.05) ${progress}% 100%)` };
    ...
    <div className="glass-card rounded-3xl overflow-hidden flex flex-col h-full max-h-[600px] group/card relative border border-white/5">
      ...
      <div className="p-5 sm:p-6 pb-2 flex justify-between items-start">
        <div className="flex-1 pr-3 min-w-0">
           <div className="flex items-center gap-2 sm:gap-3 mb-2">
             ...
             <button onClick={() => onDeleteSubject(subject.id)} className="opacity-100 lg:opacity-0 lg:group-hover/card:opacity-100 p-1.5 text-slate-400 hover:text-red-400 hover:bg-white/5 rounded-full transition-all"><Trash2 className="w-4 h-4" /></button>
           </div>
        ...
        <div className="relative w-14 h-14 sm:w-16 sm:h-16 rounded-full flex-shrink-0 flex items-center justify-center shadow-lg bg-black/20" style={donutStyle}>
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#0a1610] rounded-full flex items-center justify-center"><span className="text-xs font-bold text-white">{progress}%</span></div>
        </div>
      </div>
      ...
      <div className="px-5 sm:px-6 pb-6 flex-1 flex flex-col min-h-0">
        <ul className="space-y-3 flex-1 overflow-y-auto custom-scrollbar pr-2 -mr-2">
            ...
            return (
              <li key={topic.id} className="group flex flex-col gap-1 border-b border-white/5 pb-3 last:border-0">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-all gap-1 ml-2">
          ...
          {subject.topics.length === 0 && !isAdding && <li className="text-center text-slate-500 text-sm italic py-8">No topics yet.</li>}
        </ul>
